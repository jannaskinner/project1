...
```{R}
#mtcars %>% group_by(cyl) %>% summarize_at(c(1, 2:3,5:6), .funs=list(mean=mean,sd=sd)) %>% pivot_longer(-1) %>% separate(name, into=c("name","stat")) %>% pivot_wider() -> table1
#mtcars
#Weather %>% group_by(Location) %>% summarize_at(c(4:7), .funs=list(mean=mean,sd=sd)) %>% pivot_longer(-1) %>% separate(name, into=c("name","stat")) %>% pivot_wider() -> table1
#table1
```
